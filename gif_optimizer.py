...
def comprimir_gif_inteligente(imagenes, output_path, duracion=1000, target_mb=0.8, tipo='RGB'):
    ...
    Args:
        ...
        tipo: 'RGB', 'ThermalFalseColor' o 'SWIR_B8A'. Para los composites con
              B12/B11 se fuerza paleta 256 + MAXCOVERAGE, para preservar pixeles
              rojos de anomalia termica que ADAPTIVE/MEDIANCUT descartarian por
              ser estadisticamente raros.
    ...
    # Override para Thermal/SWIR: anomalias termicas (rojos puros) son raras pero
    # criticas. ADAPTIVE/MEDIANCUT las descartan. Forzamos 256 colores + MAXCOVERAGE
    # para garantizar que pixeles rojos extremos esten en la paleta. Incluye SWIR_B8A
    # (tambien B12/B11) ademas de ThermalFalseColor.
    es_thermal = ('thermal' in tipo.lower() or 'falso' in tipo.lower()
                  or 'swir' in tipo.lower())
    if es_thermal:
        params['colors'] = 256
        params['quality'] = max(params['quality'], 90)
        params['modo'] = params['modo'] + ' [Thermal: 256 + MAXCOVERAGE]'
        print(f"    🔥 Modo Thermal: forzando 256 colores + MAXCOVERAGE para preservar anomalias")
    ...
    for img in imagenes_procesadas:
        ...
        # Thermal usa MAXCOVERAGE (maximiza cobertura del espacio RGB, preserva
        # outliers como pixeles rojo de anomalia termica). ADAPTIVE/MEDIANCUT
        # priorizan frecuencia y descartan outliers — verificado: 0/44 vs 44/44
        # pixeles rojos preservados en Lascar 2026-04-12.
        # RGB usa ADAPTIVE (optimiza para colores mas frecuentes, OK para RGB).
        if es_thermal:
            img_p = img.quantize(colors=params['colors'], method=Image.Quantize.MAXCOVERAGE)
        else:
