...
# FIX 2026-06-14: derivar del config en vez de lista hardcodeada de 43. Antes las
# 6 vistas zoom activas (Lascar_Crater, Isluga_Crater_Fumarola, Copahue_Crater_Lake,
# Melimoyu_Conos_Eruptivos, Mentolat_Sismicidad_VT, Hudson_Ultima_Erupcion) quedaban
# SIN timelapse aunque sus PNGs si se descargaban. Ahora incluye TODAS las activas.
...
# Label que va en el overlay segun el composite. Antes estaba hardcodeado a
# "SWIR (B12-B11-B04)" para TODO lo no-RGB -> el GIF de SWIR_B8A habria mentido
# la combinacion de bandas. Ahora es por-tipo.
LABELS_TIPO = {
    'RGB': "Sentinel-2 L2A Color Verdadero",
    'ThermalFalseColor': "Sentinel-2 L2A SWIR (B12-B11-B04)",
    'SWIR_B8A': "Sentinel-2 L2A SWIR (B12-B11-B8A)",
}
...
def agregar_overlay_copernicus(img, fecha, tipo, logo_copernicus=None, buffer_km=3):
    ...
    tipo_texto = LABELS_TIPO.get(tipo, "Sentinel-2 L2A SWIR (B12-B11-B04)")
    x_tipo = 15
...
def generar_gif_ultimos_30_dias(volcan_nombre, tipo='RGB', logo_copernicus=None, buffer_km=None):
    ...
    try:
        # Para Thermal, pre-cuantizar con MAXCOVERAGE 256 antes del save del GIF.
        # MAXCOVERAGE maximiza la cobertura del espacio RGB, lo que preserva
        # outliers como los pixeles rojos puros de anomalia termica. ADAPTIVE
        # (default) y MEDIANCUT priorizan frecuencia y descartan outliers
        # (verificado: 0/44 vs 44/44 pixeles rojos en Lascar 2026-04-12).
        # Preservar anomalia roja SWIR: aplica a thermal Y a SWIR_B8A (ambos
        # tienen B12/B11 -> pixeles rojos puros de lava/fumarola caliente que
        # ADAPTIVE/MEDIANCUT descartan como outliers). MAXCOVERAGE los conserva.
        es_swir_thermal = ('thermal' in tipo.lower() or 'falso' in tipo.lower()
                           or 'swir' in tipo.lower())
        if es_swir_thermal:
            imagenes_save = [
                img.quantize(colors=256, method=Image.Quantize.MAXCOVERAGE)
                if img.mode == 'RGB' else img
                for img in imagenes
            ]
        else:
            imagenes_save = imagenes

        imagenes_save[0].save(
            output_path,
            save_all=True,
            append_images=imagenes_save[1:],
        ...
        # Mantener resolucion 800x800 uniforme con dashboard. La reduccion de
        # peso se hace despues con gif_optimizer.py, sin tocar dimensiones.
...
def main():
    ...
    for volcan in VOLCANES_ACTIVOS:
        ...
        for tipo in ['RGB', 'ThermalFalseColor', 'SWIR_B8A']:
